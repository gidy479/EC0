...
const CartPage = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto animate-fade-in-up flex flex-col lg:flex-row gap-8">
            <div className="flex-grow">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-200">
                    <h2 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-green-700 to-teal-700">
                        Shopping Cart
                    </h2>
                ...
                <div className="space-y-6">
                    {cartItems.map((item) => (
                        <div key={item.product._id} className="glass p-4 rounded-2xl flex flex-col sm:flex-row gap-6 shadow-sm border border-white/40">
                            {/* Product Image */}
                            <div className="w-24 h-24 flex-shrink-0 bg-white rounded-xl overflow-hidden self-center sm:self-start">
                            ...
                            <div className="flex-grow flex flex-col justify-between">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <Link to={`/product/${item.product._id}`} className="text-lg font-bold text-gray-800 hover:text-green-600 transition">
                                            {item.product.name}
                                        </Link>
                                        <p className="text-sm text-gray-500 mt-1">Sold by {item.product.seller?.name || 'Seller'}</p>
                                    </div>
                                    <span className="font-extrabold text-xl text-teal-700">GH₵{item.product.price.toFixed(2)}</span>
                                </div>
                ...
                <div className="mt-6 text-right">
                    <button onClick={clearCart} className="text-sm font-medium text-gray-500 hover:text-red-500 transition">
                        Clear Cart
                    </button>
                </div>
            </div>
            ...
            <div className="lg:w-96 flex-shrink-0">
                <div className="glass p-8 rounded-3xl shadow-lg border border-white/50 sticky top-24">
                    <h3 className="text-xl font-bold text-gray-800 mb-6">Order Summary</h3>
                    
                    <div className="space-y-4 mb-6">
                        <div className="flex justify-between text-gray-600">
                            <span>Subtotal ({cartCount} items)</span>
                            <span className="font-medium text-gray-900">GH₵{cartTotal.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between text-gray-600">
                    ...
                    <div className="border-t border-gray-200 pt-4 mb-8">
                        <div className="flex justify-between items-center">
                            ...
                            <span className="text-2xl font-extrabold text-teal-700">GH₵{cartTotal.toFixed(2)}</span>
                        </div>
                    </div>

                    <button 
                        onClick={handleCheckout}
                        className="w-full py-4 bg-gray-900 text-white rounded-xl font-bold text-lg shadow-xl shadow-gray-900/20 hover:shadow-gray-900/40 hover:-translate-y-1 transition transform"
                    >
                        Proceed to Checkout
                    </button>
