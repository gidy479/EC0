    ...
    return (
        <div className="animate-fade-in-up">
            <div className="flex justify-between items-center mb-10 pb-4 border-b border-gray-200/50">
                <h2 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-green-700 to-teal-700">Dashboard</h2>
                <div className="flex gap-4">
                    <button
                        onClick={() => navigate('/orders')}
                        className="glass border-green-200 text-green-700 px-5 py-2.5 rounded-xl font-bold hover:bg-green-50 transition shadow-sm"
                    >
                        My Orders
                    </button>
                    <button
                        onClick={handleLogout}
                        className="glass border-red-200 text-red-600 px-5 py-2.5 rounded-xl font-bold hover:bg-red-50 transition shadow-sm"
                    >
                        Logout
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="glass p-8 rounded-3xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-green-200 rounded-full mix-blend-multiply filter blur-2xl opacity-50 group-hover:opacity-70 transition-opacity duration-500"></div>

                    <h3 className="text-xl font-bold mb-6 text-gray-800 relative z-10 flex items-center">
                        <svg className="w-6 h-6 mr-2 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                        Profile Details
                    </h3>
                    ...
                    <div className="space-y-4 text-gray-700 relative z-10">
                        ...
                        <div className="bg-white/40 p-3 rounded-xl border border-white/50">
                            <p className="text-xs uppercase tracking-wider text-gray-500 font-bold mb-1">Email Address</p>
                            <p className="font-semibold text-gray-900 truncate">{user.email}</p>
                        </div>
                        <div className="bg-white/40 p-3 rounded-xl border border-white/50 flex justify-between items-center">
                            <div>
                                <p className="text-xs uppercase tracking-wider text-gray-500 font-bold mb-1">Account Type</p>
                                <p className="font-semibold text-gray-900">{user.role}</p>
                            </div>
                            <span className={`px-3 py-1 rounded-full text-xs font-bold shadow-sm ${user.role === 'Seller' ? 'bg-purple-100 text-purple-700 border border-purple-200' : 'bg-blue-100 text-blue-700 border border-blue-200'}`}>
                ...
                {user.role === 'Seller' && (
                    <div className="glass p-8 rounded-3xl relative overflow-hidden group">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-blue-200 rounded-full mix-blend-multiply filter blur-2xl opacity-50 group-hover:opacity-70 transition-opacity duration-500"></div>
                        ...
                        <div className="relative z-10 h-full flex flex-col justify-center items-center py-4">
                            <p className="text-sm text-gray-500 uppercase tracking-widest font-bold mb-2">From Sales</p>
                            <p className="text-5xl font-black tracking-tight text-blue-600 drop-shadow-sm">
                ...
                {/* Wallet Card */}
                <div className="glass-dark bg-gradient-to-br from-green-800 to-teal-900 p-8 rounded-3xl text-white lg:col-span-2 relative overflow-hidden shadow-xl">
                    ...
                    <div className="relative z-10 h-full flex flex-col justify-between">
                        <div className="flex justify-between items-start mb-8">
                            <h3 className="text-2xl font-bold flex items-center">
                                <svg className="w-7 h-7 mr-2 text-green-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
                                Trust Wallet
                            </h3>
                            <span className="bg-white/10 px-3 py-1 rounded-full text-xs font-medium border border-white/20 backdrop-blur-md">Secure Escrow</span>
                        </div>

                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-6 mt-auto">
                            <div>
                                <p className="text-sm text-green-100/70 uppercase tracking-widest font-bold mb-2">Available Balance</p>
                                <p className="text-5xl lg:text-6xl font-black tracking-tight text-white drop-shadow-md">
                            ...
                            <div className="w-full sm:w-auto">
                                <button
                                    ...
                                    className="w-full sm:w-auto bg-white/20 hover:bg-white text-white hover:text-green-900 px-6 py-4 rounded-2xl font-bold transition-all duration-300 backdrop-blur-md border border-white/30 shadow-[0_4px_15px_rgba(0,0,0,0.1)] flex items-center justify-center transform hover:-translate-y-1"
                                >
                                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
                                    Add Funds
                                </button>
                            </div>
                        </div>
